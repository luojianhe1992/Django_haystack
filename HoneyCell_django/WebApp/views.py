...
def add_blog(request):
    print("in the add_blog function.")
    ...
    if request.method == "GET":
        return render(request, 'WebApp/add_blog.html', context)
        ...
        if len(Blog.objects.filter(user=user, title=title)):
            errors.append("The title for this user already exist.")
            return render(request, 'WebApp/add_blog.html', context)

        if len(Blog.objects.filter(user=user, content=content)):
            errors.append("The content for this user already exist.")
            return render(request, 'WebApp/add_blog.html', context)

        new_blog_instance = Blog(user=user, title=title, content=content)
        new_blog_instance.save()
        print("Already save the new_blog_instance.")

        return render(request, 'WebApp/add_blog.html', {'user': request.user})


@login_required
def show_blogs(request):
    print("in the show_blogs function.")
    ...
    return render(request, 'WebApp/show_blogs.html', context)


@login_required
def blog_detail(request, blog_id):
    print("in the blog_detail function.")
